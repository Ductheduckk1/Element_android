    ...
    override fun onBindViewHolder(holder: PreferenceViewHolder) {
        ...
        if (isError) {
            summaryTextView?.setTextColor(
                    ThemeUtils.getColor(context, com.google.android.material.R.attr.colorError)
            )
            summaryTextView?.text = context.getString(im.vector.lib.strings.R.string.settings_notification_error_on_update)
        } else {
            summaryTextView?.setTextColor(Color.GRAY) // hoặc ThemeUtils.getColor(context, R.attr.vctr_content_secondary)
